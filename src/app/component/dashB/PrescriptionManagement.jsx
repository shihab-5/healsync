...
export default function PrescriptionManagement() {
  ...
  // Load doctors once, to resolve currentDoctor from session
  useEffect(() => {
  ...
  // Same matching pattern as AppointmentRequests
  ...
  // Fetch Prescriptions — filtered by doctor
  const loadPrescriptions = async () => {
    setLoading(true);
    try {
      const data = await getPrescriptions(currentDoctorId);
  ...
  // Wait until we know who the doctor is before fetching
  useEffect(() => {
    if (currentDoctorId) {
      loadPrescriptions();
    }
  ...
  console.log("queryAppointmentId at submit time:", queryAppointmentId); // ✅ safe here, inside a browser event handler
  console.log("current URL:", window.location.href); // ✅ safe here
    setSubmitting(true);
    const payload = {
      patientName,
      diagnosis,
